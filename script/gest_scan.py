...
class ScanWorker(QRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            if not pdf_files:
                self.signals.error.emit(self.tr("scan.no_pdf_files", "No PDF files found in the selected directories."))
                return
            
            total_files = len(pdf_files)
            logger.info(self.tr("scan.found_pdf_files", "Found {count} PDF files to process").format(count=total_files))
                ...
                try:
                    ...
                    try:
                        ...
                        if not os.path.isfile(file_path):
                            logger.warning(self.tr("scan.non_existent_file", "Skipping non-existent file: {file}").format(file=file_path))
                            continue
                            
                        # Get file info and hashes
                        file_info = get_pdf_info(file_path)
                        if not file_info:  # If get_pdf_info returns None (error case)
                            logger.warning(self.tr("scan.error_extracting_info", "Could not extract info from {file}").format(file=file_path))
                            continue
                        ...
                        if self.temp_dir is None or not os.path.isdir(self.temp_dir):
                            ...
                            logger.debug(self.tr("scan.created_temp_dir", "Created temporary directory: {dir}").format(dir=self.temp_dir))
                        ...
                        if extract_first_page_image(file_path, image_path):
                            try:
                                ...
                                logger.warning(self.tr("scan.error_calculating_image_hash", "Could not calculate image hash for {file}: {error}").format(file=file_path, error=str(e)))
                            finally:
                                ...
                                try:
                                    ...
                                    logger.warning(self.tr("scan.error_deleting_temp_file", "Could not delete temporary file {file}: {error}").format(file=image_path, error=str(e)))
                        
                        processed_files.append(file_info)
                        file_hashes[file_path] = file_hash
                        
                    except Exception as e:
                        logger.error(self.tr("scan.error_processing_file", "Error processing {file}: {error}").format(file=file_path, error=str(e)), exc_info=True)
                        continue
                    
                    logger.debug(self.tr("scan.processed_file", "Processed: {file}").format(file=file_path))
                    
                except Exception as e:
                    logger.error(self.tr("scan.error_processing_file", "Error processing {file}: {error}").format(file=file_path, error=str(e)))
                    continue
            ...
            self.signals.error.emit(
                self.tr("scan.error_during_scan", "An error occurred during scanning: {error}").format(error=str(e))
            )
        finally:
            ...
            if hasattr(self, 'temp_dir') and self.temp_dir and os.path.exists(self.temp_dir):
                try:
                    ...
                    logger.debug(self.tr("scan.cleanup_temp_dir", "Cleaned up temporary directory: {dir}").format(dir=self.temp_dir))
                except Exception as e:
                    logger.warning(
                        self.tr("scan.error_cleanup_temp_dir", "Could not remove temporary directory {dir}: {error}")
                        .format(dir=self.temp_dir, error=str(e))
                    )
...
class ScanManager(QObject):
    ...
    def start_scan(self, directories: List[str], min_similarity: float = 0.9):
        ...
        self.progress_dialog = QProgressDialog(
            self.tr("scan.scanning_message", "Scanning for duplicate PDFs..."),
    ...
    def _on_progress(self, current: int, total: int):
        ...
        if self.progress_dialog:
            ...
            self.progress_dialog.setLabelText(
                self.tr("scan.scanning_progress", "Scanning... ({current}/{total} files)")
    ...
    def _on_finished(self, duplicate_groups: list):
        ...
        logger.info(
            self.tr("scan.completed_groups", "Scan completed. Found {count} duplicate groups.")
    ...
    def _on_error(self, error_msg: str):
        ...
        QMessageBox.critical(
            None,
            self.tr("scan.error_title", "Scan Error"),
            error_msg,
