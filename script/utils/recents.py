...
class RecentFilesManager(QObject):
    ...
    def add_file(self, file_path: str, metadata: Optional[dict] = None):
        ...
        if not file_path or not (os.path.exists(file_path) and (os.path.isfile(file_path) or os.path.isdir(file_path))):
            logger.warning(
                self.tr("recents.cannot_add_nonexistent", "Cannot add non-existent path to recents: {path}")
    ...
    def _load(self):
        ...
        try:
            ...
            logger.error(
                self.tr("recents.error_loading", "Error loading recent files: {error}")
                .format(error=str(e))
            )
            self._files = []
    ...
    def _save(self):
        ...
        try:
            ...
            logger.error(
                self.tr("recents.error_saving", "Error saving recent files: {error}")
                .format(error=str(e))
            )
