    ...
    def data(self, role: int = Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if not self.file_path:
                return ""
            file_info = []
            file_info.append(self.tr("search_dup.tooltip_file", "File: {path}").format(path=self.file_path))
            if self.size is not None:
                file_info.append(self.tr(
                    "search_dup.tooltip_size", 
                    "Size: {size:,} bytes"
                ).format(size=self.size))
            if self.modified is not None:
                file_info.append(self.tr(
                    "search_dup.tooltip_modified", 
                    "Modified: {modified}"
...
class DuplicateGroup:
    ...
    def data(self, role: int = Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if count == 1:
                return self.tr("search_dup.one_duplicate", "1 duplicate file")
            return self.tr(
                "search_dup.multiple_duplicates", 
            ...
            if not self.files:
                return self.tr("search_dup.no_duplicates_in_group", "No duplicate files in this group")
            return self.tr(
                "search_dup.group_tooltip", 
...
class DuplicateFilesModel(QAbstractItemModel):
    ...
    def headerData(self, section: int, orientation: Qt.Orientation, role: int = Qt.ItemDataRole.DisplayRole):
        ...
        if orientation == Qt.Orientation.Horizontal and role == Qt.ItemDataRole.DisplayRole:
            return self.tr("search_dup.duplicate_files_header", "Duplicate Files")
        return None
    ...
    def paint(self, painter, option, index):
        ...
        try:
            ...
            if isinstance(item, DuplicateFileItem) and item.is_original:
                original_text = f" {self.tr('search_dup.original_label', '(Original)')}"
