...
class FileListWidget(QTreeWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Set up columns with localized headers
        self.setColumnCount(3)
        self.setHeaderLabels([
            self.tr("file_list.column_name", "Name"),
            self.tr("file_list.column_size", "Size"),
            self.tr("file_list.column_pages", "Pages")
        ])
    ...
    def set_duplicate_groups(self, groups):
        ...
        for i, group in enumerate(self.duplicate_groups):
            ...
            # Set group item text with localized strings
            ...
            page_text = self.tr("file_list.pages_single", "{count} page") if group[0].page_count == 1 else \
                       self.tr("file_list.pages_multiple", "{count} pages")
            group_item.setText(2, page_text.format(
                count=group[0].page_count if hasattr(group[0], 'page_count') else '?'
            ))
            ...
            tooltip = self.tr(
                ...
                savings=f"<br>{self.tr('file_list.space_savings', 'Space savings: {savings}').format(savings=self.format_file_size(space_savings))}" 
                        if len(group) > 1 else ""
            )
            ...
            for doc in group:
                ...
                # Localize pages text
                page_text = self.tr("file_list.pages_single", "{count} page") if doc.page_count == 1 else \
                           self.tr("file_list.pages_multiple", "{count} pages")
                doc_item.setText(2, page_text.format(
                    count=doc.page_count if hasattr(doc, 'page_count') else '?'
                ))
    ...
    def format_file_size(self, size_bytes):
        ...
        if size_bytes < 1024:
            return self.tr("file_list.size_bytes", "{size} B").format(size=size_bytes)
        elif size_bytes < 1024 * 1024:
            return self.tr("file_list.size_kb", "{size:.1f} KB").format(size=size_bytes / 1024)
        elif size_bytes < 1024 * 1024 * 1024:
            return self.tr("file_list.size_mb", "{size:.1f} MB").format(size=size_bytes / (1024 * 1024))
        else:
            return self.tr("file_list.size_gb", "{size:.1f} GB").format(size=size_bytes / (1024 * 1024 * 1024))
